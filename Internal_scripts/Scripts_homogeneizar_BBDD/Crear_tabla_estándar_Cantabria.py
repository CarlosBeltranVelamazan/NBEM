...
def CantabriaDB(archivo):
        ...
        sepcol = df["DIRECCION INMUEBLE"].str.split('-39', expand=True)
        sepcol.columns = ['calle', 'CP y municip', 'nose']
        first_column = sepcol.pop('nose')
        sepcol2 = sepcol["CP y municip"].str.split(' ', expand=True)
        sepcol2.columns = ['CP', 'Municip', '1','2','3','4']
        eleventh_column = sepcol2.pop('CP')
        sepcol3 = "39" + eleventh_column.astype(str)
        df.insert(1, 'CP', sepcol3)
        df.insert(2, 'CCAA', "Cantabria")
        df.insert(3, 'PROV', "Cantabria") 
        df.insert(4, 'CMUN', "") 
        eleventh_column = sepcol["CP y municip"].str[4:]
        df.insert(5, 'Municipio', eleventh_column)
        df.insert(6, 'Coordenadas_latitud', "") 
        df.insert(7, 'Coordenadas_longitud', "") 
        fifth_column = df.pop('ZONA CLIMATICA')
        df.insert(8, 'ZonaClimatica', fifth_column) 
        fifth_column = df.pop('Nº CERTIFICADO')
        ...
        eleventh_column = df.pop('CALIFICACIÓN CALEFACCIÓN')
        ...
        eleventh_column = df.pop('CALIFICACIÓN REFRIGERACIÓN')
        ...
        eleventh_column = df.pop('CALIFICACIÓN ACS')
        ...
        eleventh_column = df.pop('CALIFICACIÓN ILUMINACIÓN')
