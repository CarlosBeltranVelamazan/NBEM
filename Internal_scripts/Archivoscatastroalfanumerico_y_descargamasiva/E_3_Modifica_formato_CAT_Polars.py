...
def TratarCAT (Carpeta_archivos_descargas_CA, carpeta, fichero, Carpeta_archivos_guardar):
    ...
    cols_borrar = list(set(column_names) ^ set(colsfinca))
    finca = finca.drop(cols_borrar)

    colsuc=['Tipo_de_registro', 'Provincia', 'CMunicipioDGC', 'Clase', 'ReferenciaCatastral_parcela','C2','NombreProvincia', 'CMunicipioINE', 'NombreMunicipio','FechaConstruccion','ExactitudFechaConstruccion','Superficie_suelo_EC','Longitud_de_fachada']
    cols_borrar = list(set(column_names) ^ set(colsuc))
    unidadconstructiva = unidadconstructiva.drop(cols_borrar)
    unidadconstructiva = unidadconstructiva.rename({'C2':'Codigo_Unidad_Constructiva'})
