...
def AgruparCATporEdificio (finca, unidadconstructiva, construccion, inmueble, Carpeta_archivos_guardar, nombre_archivo):

     #print ("Empieza E_4 con " + str(nombre_archivo)) 

     carpeta_leer = Carpeta_archivos_guardar

     finca = finca.to_pandas()
     unidadconstructiva = unidadconstructiva.to_pandas()
     ...
     # def asignar_tipo_con_mayor_superficie(group):
     #      print(group)
     #      # Check if there is more than one unique value in 'Superficie_del_local'
     #      if group['Superficie_del_local'].nunique() > 1:
     #           # Encontrar el índice del dato con la mayor superficie
     #           indice_max_superficie = group['Superficie_del_local'].idxmax()
               
     #           # Obtener el valor de la columna 'Tipo' correspondiente al índice encontrado
     #           tipo_max_superficie = group.loc[indice_max_superficie, 'Tipologia_constructiva']
               
     #           # Asignar el valor de 'Tipo' en la nueva columna 'Tipo_con_mayor_superficie'
     #           group['Tipo_con_mayor_superficie'] = tipo_max_superficie
     ...
     edificios_C = df.assign(
                    ...
                    ).groupby(['ReferenciaCatastral_parcela']) \
                    .agg(ReferenciaCatastral_parcela = ('ReferenciaCatastral_parcela', 'first'), \
                         Indicador_reforma_o_rehabilitacion = ('Indicador_reforma_o_rehabilitacion', 'sum'), \
                        ...
                        # Superficie_del_local = ('Superficie_del_local', 'sum'), \
                         ...
                         Tipologia_constructiva = ('Tipologia_constructiva', 'first'), \
                         S_Viv = ('S_Viv', 'sum'), \
     ...
     # .apply(asignar_tipo_con_mayor_superficie, axis=1)
     ...
     ).groupby('ReferenciaCatastral_parcela').agg({'ReferenciaCatastral_parcela':'first', 'FechaConstruccion_BI':'mean', 'Plantas':'max', 'N_BI':'sum', 'Sup_elementos_urbanos':'sum',
                                                  'Viv':'sum', 'Almacen':'sum',
                                                  'Ind':'sum', 'Of':'sum',
                                                  'Com':'sum', 'Dep':'sum',
                                                  'Esp':'sum', 'Host':'sum',
