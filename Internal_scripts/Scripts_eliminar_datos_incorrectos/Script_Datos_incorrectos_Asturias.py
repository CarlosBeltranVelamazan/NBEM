def Eliminar_errores(archivo, eliminar_errores_ReferenciaCatastral, eliminar_errores_CP, eliminar_errores_Consumo_energía_primaria, eliminar_errores_Calificación_consumo_energía, eliminar_errores_Emisiones_CO2, eliminar_errores_Calificación_emisiones, eliminar_errores_FechaConstrucción, eliminar_errores_SuperficieUtil, ccaa):
    ...
    if eliminar_errores_ReferenciaCatastral == 1:
        ...
        # La solución anterior, la dejo de forma puramente como apunte
        ...
        # fallos2 = df.loc[df.loc[:, 'ReferenciaCatastral'] == '0'] # Copia los valores con un 0 en la referencia catastral
        # df = df.loc[df.loc[:, 'ReferenciaCatastral'] != '0'] # Quita los valores con un 0 en la referencia catastral
        # fallos3 = df.loc[df.loc[:, 'ReferenciaCatastral'] == '-']
        # df = df.loc[df.loc[:, 'ReferenciaCatastral'] != '-']
        # fallos4 = df.loc[df.loc[:, 'ReferenciaCatastral'] == '_']
        # df = df.loc[df.loc[:, 'ReferenciaCatastral'] != '_']
    ...
    if eliminar_errores_SuperficieUtil == 1:
        ...
        first_column = first_column.replace(",", ".").astype(float,errors='ignore')
        first_column = pd.to_numeric(first_column, errors='coerce')
    ...
    if eliminar_errores_Consumo_energía_primaria == 1:
        ...
        fallos3 = df.loc[df.loc[:, 'Consumo_energía_primaria'] == 'A']
        df = df.loc[df.loc[:, 'Consumo_energía_primaria'] != 'A']
        fallos4 = df.loc[df.loc[:, 'Consumo_energía_primaria'] == 'B']
        df = df.loc[df.loc[:, 'Consumo_energía_primaria'] != 'B']
        fallos5 = df.loc[df.loc[:, 'Consumo_energía_primaria'] == 'C']
        df = df.loc[df.loc[:, 'Consumo_energía_primaria'] != 'C']
        fallos6 = df.loc[df.loc[:, 'Consumo_energía_primaria'] == 'D']
        df = df.loc[df.loc[:, 'Consumo_energía_primaria'] != 'D']
        fallos7 = df.loc[df.loc[:, 'Consumo_energía_primaria'] == 'E']
        df = df.loc[df.loc[:, 'Consumo_energía_primaria'] != 'E']
        fallos8 = df.loc[df.loc[:, 'Consumo_energía_primaria'] == 'F']
        df = df.loc[df.loc[:, 'Consumo_energía_primaria'] != 'F']
        fallos9 = df.loc[df.loc[:, 'Consumo_energía_primaria'] == 'G']
        df = df.loc[df.loc[:, 'Consumo_energía_primaria'] != 'G']
        ...
        fallos = pd.concat([fallos, fallos2, fallos3, fallos4, fallos5, fallos6, fallos7, fallos8, fallos9, fallos10, fallos11], axis=0)
    j = df.shape[0]
    k = h-j
