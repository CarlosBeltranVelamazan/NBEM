...
def AlicanteDB(archivo):
        ...
        df.insert(0, 'CCAA', "Comunidad Valenciana") 
        df.insert(1, 'PROV', "Alicante") 
        df.insert(2, 'Fecha_registro', df ['VALIDO_HASTA'].str[6:10])
        df.Fecha_registro = df.Fecha_registro.astype(int) -10
        df.insert(3, 'C_CCAA', 10) 
        df.insert(4, 'CPROV', 3) 
...
def ValenciaDB(archivo):
        ...
        df = (pd.read_excel(archivo, skiprows=0)
                .dropna(how='all', axis=1))
        df.insert(0, 'CCAA', "Comunidad Valenciana") 
        df.insert(1, 'PROV', "Valencia") 
        df.insert(2, 'Fecha_registro', df ['VALIDO_HASTA'].str[6:10])
        df.Fecha_registro = df.Fecha_registro.astype(int) -10
        df.insert(3, 'C_CCAA', 10) 
        df.insert(4, 'CPROV', 46) 
        df.insert(5, 'N_certif', 1) 
        eleventh_column = df.pop('CONSUMO_EP_LETRA')
        df.insert(6, 'Calificación_consumo_energía', eleventh_column)
        eleventh_column = df.pop('EMISIONES_CO2_LETRA')
...
def CastellónDB(archivo):
        ...
        df = (pd.read_excel(archivo, skiprows=0)
                .dropna(how='all', axis=1))
        df.insert(0, 'CCAA', "Comunidad Valenciana") 
        df.insert(1, 'PROV', "Castellón") 
        df.insert(2, 'Fecha_registro', df ['VALIDO_HASTA'].str[6:10])
        df.Fecha_registro = df.Fecha_registro.astype(int) -10
        df.insert(3, 'C_CCAA', 10) 
        df.insert(4, 'CPROV', 12) 
        df.insert(5, 'N_certif', 1) 
        eleventh_column = df.pop('CONSUMO_EP_LETRA')
        df.insert(6, 'Calificación_consumo_energía', eleventh_column)
        eleventh_column = df.pop('EMISIONES_CO2_LETRA')
